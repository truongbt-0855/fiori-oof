sap.ui.define([
  ...
  return Controller.extend('com.internal.oof.controller.SODetail', {
    ...
    onDeleteDetailPress: function () {
      ...
      MessageBox.confirm(
        `Are you sure you want to delete Sales Order ${sSalesOrder}?`,
        function(oAction) {
          if (oAction === MessageBox.Action.OK) {
            // Call delete API function
            that._deleteSalesOrder(sId);
    ...
    _deleteSalesOrder: function (sId) {
      const sUrl = `https://803f6caftrial-dev-oof-backend-srv.cfapps.us10-001.hana.ondemand.com/odata/v4/admin/SalesOrders/${sId}`;
      
      // Show loading
      MessageToast.show('Deleting Sales Order ' + sId + '...');
      ...
      .then(response => {
        ...
        MessageToast.show('Sales Order ' + sId + ' deleted successfully!');
      })
      .catch(error => {
        MessageBox.error('Failed to delete sales order: ' + error.message);
      });
    },
